...
async fn main() -> Result<()> {
    ...
    let test_url =
        Url::parse("https://dldir1.qq.com/qqfile/qq/QQNT/897bf087/QQ9.9.7.21484_x64.exe")?;
    let (mut downloader, (_, ..)) = HttpDownloaderBuilder::new(test_url, save_dir)
    ...
    tokio::spawn({
        ...
        async move {
            loop {
                ...
                downloader
                    .change_connection_count(NonZeroU8::new(1).unwrap())
                    .unwrap();
                tokio::time::sleep(Duration::from_secs(2)).await;
                tracing::info!("Change connection count to 8");
                downloader
                    .change_connection_count(NonZeroU8::new(8).unwrap())
                    .unwrap();
                tokio::time::sleep(Duration::from_secs(2)).await;
                tracing::info!("Change connection count to 4");
                downloader
                    .change_connection_count(NonZeroU8::new(4).unwrap())
                    .unwrap();
            }
        }
    });
    tokio::spawn({
        ...
        async move {
            loop {
                ...
                downloader
                    .change_chunk_size(NonZeroUsize::new(1024 * 1024 * 1).unwrap())
                    .unwrap();
                tokio::time::sleep(Duration::from_secs(4)).await;
                tracing::info!("change_chunk_size to 8M");
                downloader
                    .change_chunk_size(NonZeroUsize::new(1024 * 1024 * 8).unwrap())
                    .unwrap();
                tokio::time::sleep(Duration::from_secs(4)).await;
                tracing::info!("change_chunk_size to 4M");
                downloader
                    .change_chunk_size(NonZeroUsize::new(1024 * 1024 * 4).unwrap())
                    .unwrap();
            }
        }
    });
    tokio::spawn({
        ...
        async move {
            ...
            while let Some(item) = chunks_stream.next().await {
                println!("chunk count :{}", item.chunk_size);
            }
        }
    });
